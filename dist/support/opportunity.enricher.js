 ...
 */
function getNonIntroducerUserIds(data) {
 ...
 */
function shareIntentIds(newData, existing) {
 ...
 */
function mergeInterpretation(newData, existingList) {
    const reasoning = newData.interpretation.reasoning;
    let maxConf = typeof newData.interpretation.confidence === 'number'
 ...
 */
export async function enrichOrCreate(database, embedder, newData, options) {
    ...
    if (remaining.length > 0) {
        ...
        for (const opp of remaining) {
            ...
            if (newReasoning.length >= MIN_REASONING_LENGTH_FOR_EMBEDDING &&
        ...
        if (embeddable.length > 0) {
            try {
                ...
                const vectors = (await embedder.generate(textsToEmbed));
    ...
    const mergedConfidence = typeof mergedInterpretation.confidence === 'number'
...
//# sourceMappingURL=opportunity.enricher.js.map