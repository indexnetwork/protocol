...
const getConfidence = (opp) => {
    const fromInterp = opp.interpretation?.confidence;
    if (typeof fromInterp === 'number' && !Number.isNaN(fromInterp))
        return fromInterp;
    ...
    if (typeof fromRow === 'number' && !Number.isNaN(fromRow))
        return fromRow;
...
const pickDisplayCounterpartActor = (opportunity, viewerId) => {
    const candidates = opportunity.actors.filter((actor) => actor.userId !== viewerId && actor.role !== 'introducer');
    ...
    createGraph() {
        ...
        const loadOpportunitiesNode = async (state) => {
            return timed("HomeGraph.loadOpportunities", async () => {
                ...
                try {
                    ...
                    const visibleForFeed = visible.filter((opp) => isActionableForViewer(opp.actors, opp.status, state.userId));
        ...
        const checkPresenterCacheNode = async (state) => {
            return timed("HomeGraph.checkPresenterCache", async () => {
                ...
                try {
                    const keys = opportunities.map((opp) => `home:card:${opp.id}:${userId}`);
                    const results = await this.cache.mget(keys);
                    const cachedCards = new Map();
                    const uncachedOpportunities = [];
            ...
            return timed("HomeGraph.generateCardText", async () => {
                ...
                for (let i = 0; i < opportunities.length; i += PRESENTATION_CONCURRENCY) {
                    ...
                    const chunkCards = await Promise.all(chunk.map(async (opportunity, offset) => {
                        ...
                        const reasoningSnippet = (typeof opportunity.interpretation?.reasoning === 'string'
                        ...
                        try {
                            const ctx = await gatherPresenterContext(db, opportunity, state.userId, otherActor?.userId);
                            const homeInput = {
                                ...ctx,
        ...
        const cachePresenterResultsNode = async (state) => {
            return timed("HomeGraph.cachePresenterResults", async () => {
                ...
                try {
                    await Promise.all(newCards.map((card) => this.cache.set(`home:card:${card.opportunityId}:${userId}`, card, { ttl: HOME_CACHE_TTL })));
                }
                catch (e) {
...
//# sourceMappingURL=home.graph.js.map