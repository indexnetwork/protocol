...
export function createProfileTools(defineTool, deps) {
    ...
    const createUserProfile = defineTool({
        ...
        handler: async ({ context, query }) => {
            ...
            if (name || location || hasSocialsFromQuery) {
                const socialsUpdate = {};
                if (linkedinUrl)
                    socialsUpdate.linkedin = linkedinUrl;
                if (githubUrl)
                    socialsUpdate.github = githubUrl;
                if (twitterUrl)
                    socialsUpdate.x = twitterUrl;
                if (websites?.length)
                    socialsUpdate.websites = websites;
    ...
    const updateUserProfile = defineTool({
        ...
        description: "Updates the user's existing profile. For the current user's profile, profileId can be omitted and the tool will use their profile. Use ONE call per request with all changes in action (and details if needed). For profile URLs call scrape_url first, then pass scraped content in details.",
        querySchema: z.object({
        ...
        handler: async ({ context, query }) => {
            ...
            const queryResult = await graphs.profile.invoke({ userId: context.userId, operationMode: 'query' });
    ...
    const completeOnboarding = defineTool({
        ...
        description: "Marks onboarding as complete. Call this ONLY after the user has explicitly confirmed their profile is correct. Do NOT call this until the user says 'yes', 'looks good', 'that's right', or similar confirmation.",
        querySchema: z.object({}),
    ...
    return [readUserProfiles, createUserProfile, updateUserProfile, completeOnboarding];
}
