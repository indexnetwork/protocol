...
async function enrichOpportunities(input) {
    const { opportunities, database, userId, chatSessionId, minimalForChat, presenter, useHomeCardFormat, debugSteps, existingOpportunityIds, targetUserId, } = input;
    const baseEnrichedRaw = await Promise.all(opportunities.map(async (opp) => {
        ...
        if (candidateUser && !candidateUser.isGhost && !candidateUser.onboarding?.completedAt && !isDirectTarget)
            return null;
    ...
    const enriched = baseEnriched.map((item, idx) => {
        ...
        return {
            ...
            matchReason: truncateForChat(item.opportunity.interpretation?.reasoning ?? "") ?? "",
...
export async function runDiscoverFromQuery(input) {
    const { opportunityGraph, database, userId, query, indexScope, limit = 5, triggerIntentId, targetUserId, onBehalfOfUserId, chatSessionId, } = input;
    if (indexScope.length === 0) {
        return {
            found: false,
            count: 0,
            message: "You need to join at least one index (community) to discover opportunities. Use read_indexes to see available indexes, or create one.",
        };
    }
    const debugSteps = [];
